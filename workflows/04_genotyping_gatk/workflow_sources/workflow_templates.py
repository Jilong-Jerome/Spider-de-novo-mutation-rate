...
def callable_sites(work_path,script_path,data_path,log_path,sp,female,male,offspring,chrom,minDP):
    ...
    python {script_path}/callable_GQ_DP_filter_minDP.py {offspring}_{chrom}_raw.recode.vcf {data_path}/{sp}_per_ind_per_chrom_DP.tsv {offspring}_{chrom}_callable_sites.vcf {minDP}
...
def dnm_scan(work_path,script_path,data_path,log_path,sp,female,male,offspring,chrom,minDP):
    ...
    python {script_path}/germline_scan.py {offspring}_{chrom}_SNPs.recode.vcf {male} {female} {offspring} {offspring}_{chrom}_putative_DNMs.vcf
    ...
    python {script_path}/germline_scan_AB.py {offspring}_{chrom}_putative_DNMs_distance_filtered.vcf {male} {female} {offspring} {offspring}_{chrom}_putative_DNMs_distance_filtered_AB.vcf
    echo done > {outputs["log"]}
    date
    jobinfo $SLURM_JOBID
    """
...
def het_AD_retrive(work_path,script_path,data_path,log_path,female,male,offspring,chrom,sp,minDP):
    inputs = {"log":f"{log_path}/germline_step2/minDP_{minDP}/{offspring}_{chrom}_minDP_{minDP}_DNM_scan.DONE"}
    outputs = {"log":f"",
            "AD":f"{work_path}/{sp}/DNMs/minDP_{minDP}/{offspring}/chrom_splits/{offspring}_{chrom}_AD_count_per_ind.tsv",
            "log":f"{log_path}/germline_step2/minDP_{minDP}/{offspring}_{chrom}_minDP_{minDP}_AD_retrive.DONE"}
    options = {
              'cores': 1,
              'memory': '4g',
              'walltime':'12:00:00',
    ...
    mkdir -p {work_path}/{sp}/DNMs/minDP_{minDP}/{offspring}/chrom_splits
    cd {work_path}/{sp}/DNMs/minDP_{minDP}/{offspring}/chrom_splits
    python {script_path}/ind_AD_retrive.py {offspring}_{chrom}_SNPs.recode.vcf {offspring}_{chrom}_AD_count_per_ind.tsv {female} {male} {offspring} {sp} {minDP}
    echo done > {outputs["log"]}
    date
    jobinfo $SLURM_JOBID
    """
...
def dnm_check(work_path,script_path,data_path,bam_path,log_path,sp,ref,female,male,offspring,chrom,minDP):
    ...
    python {script_path}/germline_scan.py {offspring}_{chrom}_DNM_region_bcftools.vcf {male} {female} {offspring} {offspring}_{chrom}_DNM_surrounding_region.vcf
    python {script_path}/bcftools_distance_filter.py {inputs["vcf"]} {offspring}_{chrom}_DNM_surrounding_region.vcf {offspring}_{chrom}_DNM_bcftools_dist_checked.vcf
     ...
     python {script_path}/bam_check.py {offspring}_{chrom}_DNM_bcftools_AD_check.tsv {offspring}_{chrom}_bam_check_pass.tsv
...
def dnm_igv_script(work_path,script_path,bam_path,log_path,sp,minDP):
    ...
    inputs = [f"{work_path}/{sp}/DNMs/minDP_{minDP}/{sp}_minDP_{minDP}_automate.tsv".replace("/Users/au688344/GenomeDK","/home/jilong")]
...
def ind_het_est(work_path,script_path,log_path,sp,offspring,chrom,minDP):
    inputs = {"callable_vcf":f"{work_path}/{sp}/callable/minDP_{minDP}/{offspring}/chrom_split/{offspring}_{chrom}_callable_sites.vcf",
            "log": f"{log_path}/germline_step2/minDP_{minDP}/{offspring}_{chrom}_minDP_{minDP}_callable_sites.DONE"}
    outputs = {
            "log": f"{log_path}/germline_step2/minDP_{minDP}/{offspring}_{chrom}_minDP_{minDP}_ind_het.DONE"
            }
    options = {
              'cores': 1,
              'memory': '4g',
    ...
    mkdir -p {work_path}/{sp}/callable/minDP_{minDP}/{offspring}/chrom_split
    cd {work_path}/{sp}/callable/minDP_{minDP}/{offspring}/chrom_split
    python {script_path}/ind_het_count.py {offspring}_{chrom}_callable_sites.vcf {sp} {chrom} {minDP} {offspring}_{chrom}.het
    echo done > {outputs["log"]}
    date
    jobinfo $SLURM_JOBID
    """
