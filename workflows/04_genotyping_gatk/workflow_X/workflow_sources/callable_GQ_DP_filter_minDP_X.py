...
# Load the sex information per offspring
...
def filter_vcf_line(line, header, mean_depth_info, minDP,off_sex):
    fields = line.split('\t')
    chromosome = fields[0]
    if fields[6] != "PASS":
        return False
    ...
    for sample_field, sample_header in zip(sample_fields, header[9:]):
        if "_F_" in sample_header:
            sample_scaler = 1
        elif "_M_" in sample_header:
            sample_scaler = 0.5
        else:
            sample_sex = off_sex[sample_header]
        ...
        if not ((0.5 * mean_depth <= dp <= 2 * mean_depth) and  (dp >= minDP*sample_scaler)):
...
def filter_vcf(vcf_filepath, mean_depth_filepath, output_filepath, sex_file_path, minDP):
    mean_depth_info = load_mean_depth_info(mean_depth_filepath)
    off_sex = load_offspring_sex_info(sex_file_path)
    with open_file_autodetect(vcf_filepath, 'rt') as vcf_file:
        ...
        with open_file_autodetect(output_filepath, 'wt') as output_file:
            ...
            for line in vcf_file:
                if filter_vcf_line(line, header_lines[-1].strip().split('\t'), mean_depth_info, minDP, off_sex):
...
sex_file_path = sys.argv[4]# path to the offspring sex information
minDP_threshold = int(sys.argv[5])
filter_vcf(vcf_filepath, mean_depth_filepath, output_filepath,sex_file_path, minDP_threshold)
