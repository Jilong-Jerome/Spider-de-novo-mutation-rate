...
def filter_vcf_line(line, header, mean_depth_info):
    fields = line.split('\t')
    chromosome = fields[0]
    if fields[6] != "PASS":
        return False
    ...
    for sample_field, sample_header in zip(sample_fields, header[9:]):
        ...
        if not (0.5 * mean_depth <= dp <= 2 * mean_depth):
            return False  # Filter out this site if DP is not within range
...
def filter_vcf(vcf_filepath, mean_depth_filepath, output_filepath):
    ...
    with open_file_autodetect(vcf_filepath, 'rt') as vcf_file:
        ...
        with open_file_autodetect(output_filepath, 'wt') as output_file:
            ...
            for line in vcf_file:
                if filter_vcf_line(line, header_lines[-1].strip().split('\t'), mean_depth_info):
...
filter_vcf(vcf_filepath, mean_depth_filepath, output_filepath)
